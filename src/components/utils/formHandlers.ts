...
import { FormInputErrorStep3, User } from "@/types";
...
import { _companyImage } from "@/types/CostilMedina";
...
export const handleSubmitStep3 = async (
  ...
  if (!validateAllFields(state, setErrorsStep3)) {
    ...
    try {
      const response = await fetch("http://74.242.168.235/api/Client/UpdateData", {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...
          myCompanies: state.companyForms,
        }),
      });
...
export const handleChangePatronymic = (
  ...
  sessionStorage.setItem('patronimic', e.target.value);
};
